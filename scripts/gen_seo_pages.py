...
def page(title,desc,canonical,ld,bodyhtml,ogt,robots="index,follow"):
    return ("<!DOCTYPE html><html lang=ko><head><meta charset=UTF-8>"
      ...
      f"<link rel=canonical href='{canonical}'><meta name=robots content='{robots}'>"
...
for p in D["players"]:
    ...
    open(f"p/{pid}.html","w",encoding="utf-8").write(page(title,desc,f"https://kicktalk.xyz/p/{pid}.html",ld,body,f"{name} — {p['team']} | 킥톡",robots="noindex,follow"))
    np+=1
nt=0
for t in D["teams"]:
    tid=t["id"]; tn=t["name"]
