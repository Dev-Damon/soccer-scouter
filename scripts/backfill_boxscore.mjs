...
for (const fx of finished) {
  try {
    // 증분: 이미 통계 있는 정적 파일이면 스킵(cron이 새 경기만 처리해 가볍게)
    const luPath = join(ROOT, "lu", fx.id + ".json");
    if (existsSync(luPath)) { try { const ex = JSON.parse(readFileSync(luPath, "utf8")); if (ex.boxscore && ex.boxscore.teams && (ex.boxscore.teams[0].statistics || []).length > 0) { skip++; continue; } } catch {} }
